function [TrueRandom, I_notation, finalEncrptedTXT, PrivateKey] = serverThing_again(mask_vec)
...
%%Modification between method and method
sram_vec = double(sram_vec);
sram_vec(mask_vec==1) = 2;
...
for index = 1:32
    ...
    for indexx = 1:16
        [temp,~,x,y] = access_Cell(subAddressList, sram_mat);
        while(temp == 2)
            if (x == 64)
                x = 1;
                y = y+1;
        ...
        Cipher(index, indexx) = temp;
        subAddressList = instructionOp(subAddressList, temp);
    end
end
...
function [ret_val, op, add_x, add_y] = access_Cell(subAddressList, sram_mat)
...
switch op
    ...
    case 1
        new_sub_add = circshift(subAddressList, 1);
    case 2
        new_sub_add = circshift(subAddressList, -1);
end
end
